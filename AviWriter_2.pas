...
begin
  ...
  try
    ...
    if (AVIFileCreateStream(pfile, pstream, StreamInfo) <> AVIERR_OK) then
      exit;//raise Exception.Create('Failed to create AVI video stream');
    ...
    try
      ...
      for i := 0 to Bitmaps.Count - 1 do begin
        ...
        try
          ...
          if (BitmapInfoSize = 0) then
            exit;//raise Exception.Create('Failed to retrieve bitmap info');
          ...
          if i = 0 then
            if (AVIStreamSetFormat(pstream, 0, BitmapInfo, BitmapInfoSize) <> AVIERR_OK) then
              exit;//raise Exception.Create('Failed to set AVI stream format');
          ...
          if AVIERR <> AVIERR_OK then
            exit//raise Exception.Create('Failed to add frame to AVI.')
        finally
      ...
      if CreateEditableStream(VideoStream, pstream) <> AVIERR_OK then
        exit;//raise Exception.Create('Could not create Video Stream');
    finally
...
begin
 ...
 try
    AVIERR:=
      ...
      Samples_Written,Bytes_Written);
 except AVIERR:= AVIERR_ERROR end;
 inc(fFrameCount);

 if Assigned(fOnProgress) then
  if (fFrameCount mod 1 = 0) then
...
begin
  ...
  if fPixelFormat = pfDevice then begin
   fAbort := true;
   exit
  end;

  if fCompOnFly then Workfile:= fFilename
   else Workfile:= TempFileName;
...
begin
 ...
 if fAbort then Result:= AVIERR_USERABORT
  else result:= AVIERR_OK
end;
    ...
    begin
      ...
      if FileExists(filename) then
        if not Deletefile(filename) then
          raise Exception.Create('File ' + ExtractFileName(filename) + ' could not be deleted');
...
begin
  ...
  if ic <> 0 then begin
   fFourCC:= FourCC;
   ICClose(ic)
  end else raise Exception.Create('No compressor for ' + FourCC + ' available')
end;
...
begin
 if fAbort then exit;
 if not fInitialized then exit;//raise Exception.Create('Video not initialized');

 if (fFourCC = '') or (not fCompOnFly) then begin
  AddFrame(ABmp);

  for i := 1 to (Showtime div FrameTime) do begin
      AVIERR:=
  ...
  begin
    ...
    //need to force the same for all
      ...
      for i := 0 to (Showtime div FrameTime) do begin //for i := 0 to Showtime-fFrameCount do
       if fAbort then exit;
...
begin
  ...
  if (AVIStreamSetFormat(fCompStream, 0, fPInInfo, fInInfoSize) <> AVIERR_OK) then exit
...
begin
 if LowerCase(ExtractFileExt(filename)) <> '.wav'
  then raise Exception.Create('WavFileName must name a file '
      + 'with the .wav extension')
  else fWaveFileList.AddObject(filename, TObject(Delay))
end;
...
begin
 if fAbort then exit;
 if not fInitialized then raise Exception.Create('Video not initialized');
